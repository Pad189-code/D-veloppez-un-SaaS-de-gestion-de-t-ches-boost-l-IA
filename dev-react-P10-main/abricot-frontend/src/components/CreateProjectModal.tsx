...
export default function CreateProjectModal({
  ...
  /** Figma : champs une ligne, hauteur 48px, même style que les modales tâche */
  const fieldClass =
  ...
  return (
    <div
      ...
      role="presentation"
    >
      {/* Figma : 598×616 px, padding 73 / 79, contenu sans scroll dans le cadre */}
      <div
        ref={modalRef}
        tabIndex={-1}
        className="create-project-modal box-border relative flex min-h-0 h-[min(616px,calc(100vh-32px))] w-full max-w-[598px] flex-col rounded-[10px] border border-[#E0E0E0] bg-white px-[73px] pb-[79px] pt-[79px] shadow-none outline-none sm:h-[616px] sm:w-[598px] sm:max-w-[598px] max-sm:px-5 max-sm:pb-10 max-sm:pt-10"
      ...
      >
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col gap-8" noValidate>
          ...
          {/* Figma : bouton à gauche, largeur au texte (pas pleine largeur), gris clair + texte gris si invalide */}
          <button
            type="submit"
            disabled={!canSubmit}
